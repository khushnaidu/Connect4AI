...
def board_creating(matrix_rows: int, matrix_cols: int):
    ...
    columns_index_print = [
        "-" + str(x) + "-" if x < 10 else str(x) for x in range(1, number_of_cols + 1)
    ]
...
def check_free_columns(matrix_board: list, matrix_cols: int):
    free_cols = [
        (index_col + 1)
        for index_col in range(matrix_cols)
        if matrix_board[0][index_col] == " "
    ]
    return free_cols


# Take the user input and check it if is valid, return correct index for token place
def player_token_placement(
    p_symbol: str, player_name: str, free_columns_index: list, matrix
):
    if player_name == "AI":
...
# Place token, check for free spaces
...
def another_game():
    while True:
        ...
        if do_you_want_new_game.lower() == "yes" or do_you_want_new_game.lower() == "y":
            ...
            do_you_want_new_game.lower() == "no" or do_you_want_new_game.lower() == "n"
        ):
            return False
        else:
            print(f"\nIncorrect input!")
    ...
    for game_number in range(100):
        ...
        for counter in range(0, 100):
            ...
            mc_matrix, mc_r_c_last_token = place_token(
                mc_matrix, number_of_rows, random_place, player_symbol
            )
            ...
            mc_end_game_flag = (
                False
                if len(check_free_columns(mc_matrix, number_of_cols)) > 0
                else True
            )
...
def winner_print(player_name: str):
...
while not winner_flag and not end_game_flag:
    ...
    for player_name, players_symbol in players_dict.items():
        ...
        )  # Check for free columns (cols w empty on top)
        column_to_place = player_token_placement(
            players_symbol, player_name, free_columns, board
        )

        # Placing the color token in the board
        board, r_c_last_token = place_token(
            board, number_of_rows, column_to_place, players_symbol
        )

        # Check if there is a winner ( based on last placed token )
        winner_flag = winner_check(
            board, players_symbol, r_c_last_token, number_of_rows, number_of_cols
        )

        # Check if there is more empty spaces
        end_game_flag = (
            False if len(check_free_columns(board, number_of_cols)) > 0 else True
        )
    ...
    if new_game_flag:
        ...
        end_game_flag = False
        new_game_flag = False
        board, columns_print_for_representation = board_creating(
